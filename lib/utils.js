...
export const prettyFeatures = service => {
    let array = []
    if(service.promoted) array.push("<strong>Recommended</strong>")
    if(service.distance && !service.promoted) array.push(prettyDistance(service.distance))
    if(service.accessibility && service.accessibility.includes("building wheelchair access")) array.push("Wheelchair accessible")
    if(service.days && service.days.length === 7) array.push("On every day")
